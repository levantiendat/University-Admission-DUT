...
async def get_major_course_endpoint(major_course_id: int, db: Session = Depends(get_db)):
    ...
    """
    major_course = get_major_course(db=db, major_course_id=major_course_id)
    if not major_course:
...
async def update_major_course_endpoint(major_course_id: int, major_course: MajorCourseUpdate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    if user.role != "admin":
        raise ForbiddenException(detail="You do not have permission to perform this action")
    return update_major_course(db=db, major_course_id=major_course_id, major_course=major_course)
...
def get_major_course_detail_endpoint(major_course_detail_id: int, db: Session = Depends(get_db)):
    ...
    """
    major_course_detail = get_major_course_detail(db=db, major_course_detail_id=major_course_detail_id)
...
def update_major_course_detail_endpoint(major_course_detail_id: int, major_course_detail: MajorCourseDetailUpdate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    if user.role != "admin":
        raise ForbiddenException(detail="You do not have permission to perform this action")
    return update_major_course_detail(db=db, major_course_detail_id=major_course_detail_id, major_course_detail=major_course_detail)

@router.delete("/major_course_details/{major_course_detail_id}", status_code=204)
def delete_major_course_detail_endpoint(major_course_detail_id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    """
...
def create_course_prior_course_endpoint(course_prior_course: CoursePriorCourseCreate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    if user.role != "admin":
        raise ForbiddenException(detail="You do not have permission to perform this action")
    return create_course_prior_course(db=db, course_prior_course=course_prior_course)

@router.get("/course_prior_courses", response_model=list[CoursePriorCourseOut])
def get_course_prior_courses_endpoint(db: Session = Depends(get_db)):
    """
    API để lấy danh sách mối quan hệ giữa học phần và các học phần học trước
    """
    return get_course_prior_courses(db=db)

@router.get("/course_prior_courses/{course_prior_course_id}", response_model=CoursePriorCourseOut)
def get_course_prior_course_endpoint(course_prior_course_id: int, db: Session = Depends(get_db)):
    """
    API để lấy thông tin mối quan hệ giữa học phần và các học phần học trước theo ID
    """
    course_prior_course = get_course_prior_course(db=db, course_prior_course_id=course_prior_course_id)
...
def update_course_prior_course_endpoint(course_prior_course_id: int, course_prior_course: CoursePriorCourseUpdate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    """
    ...
    if user.role != "admin":
        raise ForbiddenException(detail="You do not have permission to perform this action")
    return update_course_prior_course(db=db, course_prior_course_id=course_prior_course_id, course_prior_course=course_prior_course)

@router.delete("/course_prior_courses/{course_prior_course_id}", status_code=204)
def delete_course_prior_course_endpoint(course_prior_course_id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    """
...
async def create_course_prerequisite_endpoint(course_prerequisite: CoursePrerequisiteCreate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    if user.role != "admin":
        raise ForbiddenException(detail="You do not have permission to perform this action")
    return create_course_prerequisite(db=db, course_prerequisite=course_prerequisite)
...
async def get_course_prerequisites_endpoint(db: Session = Depends(get_db)):
    ...
    """
    return get_course_prerequisites(db=db)
...
async def get_course_prerequisite_endpoint(course_prerequisite_id: int, db: Session = Depends(get_db)):
    ...
    """
    course_prerequisite = get_course_prerequisite(db=db, course_prerequisite_id=course_prerequisite_id)
...
async def update_course_prerequisite_endpoint(course_prerequisite_id: int, course_prerequisite: CoursePrerequisiteUpdate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    if user.role != "admin":
        raise ForbiddenException(detail="You do not have permission to perform this action")
    return update_course_prerequisite(db=db, course_prerequisite_id=course_prerequisite_id, course_prerequisite=course_prerequisite)
...
async def create_course_corequisite_endpoint(course_corequisite: CourseCorequisiteCreate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    if user.role != "admin":
        raise ForbiddenException(detail="You do not have permission to perform this action")
    return create_course_corequisite(db=db, course_corequisite=course_corequisite)
...
async def get_course_corequisites_endpoint(db: Session = Depends(get_db)):
    ...
    """
    return get_course_corequisites(db=db)
...
async def get_course_corequisite_endpoint(course_corequisite_id: int, db: Session = Depends(get_db)):
    ...
    """
    course_corequisite = get_course_corequisite(db=db, course_corequisite_id=course_corequisite_id)
...
async def update_course_corequisite_endpoint(course_corequisite_id: int, course_corequisite: CourseCorequisiteUpdate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    if user.role != "admin":
        raise ForbiddenException(detail="You do not have permission to perform this action")
    return update_course_corequisite(db=db, course_corequisite_id=course_corequisite_id, course_corequisite=course_corequisite)
