<template>
    <div class="admin-previous-admissions">
      <div class="admin-page-header d-flex justify-content-between align-items-center">
        <div>
          <h2 class="admin-page-title">Quản lý điểm chuẩn các năm trước</h2>
          <p class="admin-page-description">Quản lý điểm chuẩn theo ngành, phương thức và năm</p>
        </div>
        <router-link to="/admins/previous-admissions/create" class="btn-create">
          <i class="bi bi-plus-circle me-2"></i>Thêm điểm chuẩn
        </router-link>
      </div>
      ...
      <div v-if="loading" class="loading-container">
        ...
        <p class="loading-text">Đang tải danh sách điểm chuẩn...</p>
      </div>
      ...
      <div v-else class="admin-card">
        <div class="table-responsive">
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="admission in filteredAdmissions" :key="admission.id" class="data-row">
                <td><span class="id-badge">#{{ admission.id }}</span></td>
                <td>
                  <span class="major-name">{{ getMajorName(admission.major_id) }}</span>
                </td>
                <td>
                  <span class="method-name">{{ getMethodName(admission.admission_methods_id) }}</span>
                </td>
                <td><span class="year-badge">{{ admission.year }}</span></td>
                <td><span class="score-value">{{ formatScore(admission.score) }}</span></td>
                <td>
                  <div class="action-buttons">
                    <router-link :to="`/admins/previous-admissions/${admission.id}`" class="btn-action edit" title="Xem và chỉnh sửa">
              ...
              <tr v-if="filteredAdmissions.length === 0">
                <td colspan="6">
                  <div class="empty-state">
                    ...
                    <h4>Không tìm thấy điểm chuẩn nào</h4>
                    <p v-if="hasFilters">Thử thay đổi các bộ lọc</p>
                    <p v-else>Chưa có dữ liệu điểm chuẩn trong hệ thống</p>
                    <router-link to="/admins/previous-admissions/create" class="btn-create-empty">
                      <i class="bi bi-plus-circle me-2"></i>Thêm điểm chuẩn
                    </router-link>
                  </div>
                </td>
      ...
      <div v-if="showDeleteModal" class="modal-overlay" @click="cancelDelete">
        <div class="modal-content" @click.stop>
          ...
          <div class="modal-body">
            <p>Bạn có chắc chắn muốn xóa điểm chuẩn của ngành <strong>{{ getMajorName(admissionToDelete?.major_id) }}</strong> năm <strong>{{ admissionToDelete?.year }}</strong>?</p>
            <p class="text-danger">
  ...
  export default {
    ...
    setup() {
      ...
      const availableYears = computed(() => {
        const years = [...new Set(admissions.value.map(a => a.year))].sort((a, b) => b - a) // Sort years in descending order
        return years
      })
      
      // Check if any filters are applied
      const hasFilters = computed(() => {
        return filters.majorId || filters.methodId || filters.year || searchQuery.value
      })
      ...
      const applyFilters = () => {
        filteredAdmissions.value = admissions.value.filter(admission => {
          // Apply major filter
          if (filters.majorId && admission.major_id !== parseInt(filters.majorId)) {
            return false
          }
          
          // Apply method filter
          if (filters.methodId && admission.admission_methods_id !== parseInt(filters.methodId)) {
            return false
          }
          
          // Apply year filter
          if (filters.year && admission.year !== parseInt(filters.year)) {
            return false
          }
          
          // Apply search query (search by major name or method name)
          if (searchQuery.value) {
            const majorName = getMajorName(admission.major_id).toLowerCase()
            const methodName = getMethodName(admission.admission_methods_id).toLowerCase()
            const query = searchQuery.value.toLowerCase()
            
            return majorName.includes(query) || methodName.includes(query) || admission.year.toString().includes(query)
          }
      ...
      const confirmDelete = (admission) => {
        admissionToDelete.value = admission
      ...
      const cancelDelete = () => {
        ...
        admissionToDelete.value = null
      }
  
      // Delete admission
      const deleteAdmission = async () => {
        if (!admissionToDelete.value) return
        
        try {
          isDeleting.value = true
          await PreviousAdmissionController.deletePreviousAdmission(admissionToDelete.value.id)
          
          // Remove from lists
          admissions.value = admissions.value.filter(a => a.id !== admissionToDelete.value.id)
      ...
      // Load admissions, majors and methods
      const loadData = async () => {
        try {
          ...
          admissions.value = admissionsData
